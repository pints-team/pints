...
class TestPrior(unittest.TestCase):
    ...
    def test_binomial_prior(self):

        # Test input parameters
        self.assertRaises(TypeError, pints.BinomialLogPrior, 3.5, 0.5)
        self.assertRaises(ValueError, pints.BinomialLogPrior, 0, 0.5)
        self.assertRaises(ValueError, pints.BinomialLogPrior, 2, -2)
        self.assertRaises(ValueError, pints.BinomialLogPrior, 2, 2)
        ...
        self.assertAlmostEqual(p2.mean(), 5.6)

        # Test CDFs
        self.assertAlmostEqual(p1.cdf(2), 0.7801491456)
        self.assertAlmostEqual(p1.cdf(3), 0.9513573696)
        self.assertAlmostEqual(p2.cdf(5), 0.46958126185472589824)
        self.assertAlmostEqual(p2.cdf(7), 0.88887567769391857664)

        # Test inverse-CDFs
        self.assertAlmostEqual(p1.icdf(0.9), 3)
        self.assertAlmostEqual(p1.icdf(0.996), 5)
        self.assertAlmostEqual(p2.icdf(0.5), 6)
        self.assertAlmostEqual(p2.icdf(0.9), 8)
        ...
        for point in points:
            ...
            self.assertAlmostEqual(
                ...
                places=9)
            self.assertAlmostEqual(
                scipy.stats.binom.logpmf(to_test[0], 10.0, 0.56),
                p2(to_test),
                places=9)
