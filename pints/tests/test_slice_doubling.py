...
class TestSliceDoubling(unittest.TestCase):
    ...
    Please refer to the _slice_doubling.py script in
    ..\_mcmc\_slice_doubling.py
    """
    ...
    def test_first_run(self):
        ...
        self.assertTrue(mcmc._current_log_y ==
                        (mcmc._current_log_pdf - mcmc._e))
    ...
    def test_cycle(self):
        ...
        self.assertTrue(mcmc._k > 0 and (mcmc._current_log_y < mcmc._fx_l
                        or mcmc._current_log_y < mcmc._fx_r))

        # The edges are inside the slice: return None and update the log pdf
        # of the left edge
        fx = log_pdf.evaluateS1(x)[0]
        sample = mcmc.tell(fx)
        ...
        # SUBSEQUENT EXPANSIONS: expand left edge n-1 times and on the
        # nth iteration we expand the right edge
        ...
        # COMPLETE INTERVAL EXPANSION: Check whether the edges are now outside
        # the slice
        self.assertFalse(mcmc._k > 0 and (mcmc._current_log_y < mcmc._fx_l or
                         mcmc._current_log_y < mcmc._fx_r))

        # PROPOSE PARAMETER: now that we have estimated the interval, we
        # sample a new parameter, set the interval_found, _init_check and
        # _continue_check flags to True
        x = mcmc.ask()
        self.assertTrue(mcmc._interval_found)
        self.assertTrue(mcmc._init_check)
        self.assertTrue(mcmc._continue_check)

        # The log pdf of the proposed point is smaller than the slice height.
        # The ``Threshold Check`` was not passed, so we reject, shrink and set
        # the _init_check and _continue_check flags to False
        fx = log_pdf.evaluateS1(x)[0]
        sample = mcmc.tell(fx)
        self.assertEqual(sample, None)
        self.assertEqual(mcmc._l, mcmc._proposed[mcmc._active_param_index])
        self.assertFalse(mcmc._init_check)
        self.assertFalse(mcmc._continue_check)
        ...
        # START ACCEPTANCE CHECK: Since the new proposed point passed the
        # threshold test (fx > log_y), we mantain the flags for proceeding
        # with the ``Acceptance Check``
        self.assertTrue(mcmc._init_check)
        self.assertTrue(mcmc._continue_check)

        # Check whether we are initialising the ``Acceptance Check`` interval
        # appropriately
        x = mcmc.ask()
        fx = log_pdf.evaluateS1(x)[0]
        sample = mcmc.tell(fx)
        ...
        # The rejection condition in the ``Acceptance Check`` procedure should
        # be False
        self.assertFalse(mcmc._d and mcmc._current_log_y >= fx[0] and
                         mcmc._current_log_y >= fx[1])

        # Since the point hasn't been rejected, we will continue the
        # ``Acceptance Check`` process
        while ((mcmc._r_hat - mcmc._l_hat) > 1.1 *
                mcmc._w[mcmc._active_param_index]):
            x = mcmc.ask()
            fx = log_pdf.evaluateS1(x)[0]
            sample = mcmc.tell(fx)

        # The loop ends once the interval is smaller than ``1.1*w```
        self.assertFalse((mcmc._r_hat - mcmc._l_hat) > 1.1 *
                         mcmc._w[mcmc._active_param_index])

        # Since the ``Acceptance Check`` is finished, the _continue_check has
        # been set to False and we accepted the point
        x = mcmc.ask()
        self.assertFalse(mcmc._continue_check)
        fx = log_pdf.evaluateS1(x)[0]
        sample = mcmc.tell(fx)

        # As we have accepted the new point, we reset the interval expansion
        # flags
        self.assertTrue(mcmc._first_expansion)
        self.assertFalse(mcmc._interval_found)
        ...
        # Check that we are still within the slice or that k > 0
        self.assertTrue(mcmc._k > 0 and (mcmc._current_log_y < mcmc._fx_l or
                        mcmc._current_log_y < mcmc._fx_r))
        ...
        # SUBSEQUENT EXPANSIONS: expand left edge n-1 times and on the nth
        # iteration we expand the right edge
        while mcmc._v < .5 or mcmc._k == 0:
            x = mcmc.ask()
            self.assertTrue(mcmc._k > 0 and (mcmc._current_log_y < mcmc._fx_l
                            or mcmc._current_log_y < mcmc._fx_r))
        ...
        # PROPOSE PARAMETER: now that we have estimated the interval,
        # we sample a new parameter, set the interval_found, _init_check and
        # _continue_check flags to True
        x = mcmc.ask()
        self.assertTrue(mcmc._interval_found)
        self.assertTrue(mcmc._init_check)
        ...
        # Since the intervals generated from the new point do not differ from
        # the ones generated by the current sample, the following condition
        # should be false, therefore d should remain False
        self.assertFalse((mcmc._current[mcmc._active_param_index] < mcmc._m
                         and
                         mcmc._proposed[mcmc._active_param_index] >= mcmc._m)
                         or
                         (mcmc._current[mcmc._active_param_index] >= mcmc._m
                         and
                         mcmc._proposed[mcmc._active_param_index] < mcmc._m))
        ...
        # Since the point hasn't been rejected, we will continue the
        # ``Acceptance Check`` process
        while ((mcmc._r_hat - mcmc._l_hat) >
                1.1 * mcmc._w[mcmc._active_param_index]):
            x = mcmc.ask()
            fx = log_pdf.evaluateS1(x)[0]
            sample = mcmc.tell(fx)

        # The loop ends once the interval is smaller than ``1.1*w``
        self.assertFalse((mcmc._r_hat - mcmc._l_hat) >
                         1.1 * mcmc._w[mcmc._active_param_index])
        self.assertFalse((mcmc._d and mcmc._current_log_y >= fx[0] and
                         mcmc._current_log_y >= fx[1]))

        # Since the Acceptance check is finished, the _continue_check has been
        # set to False and we accepted the point
        x = mcmc.ask()
        self.assertFalse(mcmc._continue_check)
        fx = log_pdf.evaluateS1(x)[0]
        sample = mcmc.tell(fx)
        ...
        # All the parameters of the sample have been updates, so we reset the
        # index to 0
        self.assertEqual(mcmc._active_param_index, 0)

        # Now that we have generated the new sample, we set this to be the
        # current sample
        self.assertTrue(np.all(mcmc._current == mcmc._proposed))
