...
class TestDiagnostics(unittest.TestCase):
    ...
    def test_autocorrelation(self):
        # Test case I: 100% correlated
        n_chains = 3
        n_iterations = 10
        n_parameters = 2
        chains = np.ones(shape=(n_chains, n_iterations, n_parameters))
        autocorr = pints.diagnostics.autocorrelation(chains)

        self.assertEqual(
            autocorr.shape, (n_chains, n_iterations, n_parameters))
        self.assertTrue(np.alltrue(autocorr == 1))

        # Test case II: jump in samples
        chains = np.array([1, 1, 1, -1, -1, -1])[np.newaxis, :, np.newaxis]
        autocorr = pints.diagnostics.autocorrelation(chains)

        n_samples = 6
        var = 6 / (n_samples - 1)
        self.assertEqual(autocorr.shape, (1, 6, 1))
        autocorr = autocorr[0, :, 0]
        self.assertAlmostEqual(autocorr[0], 6 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[1], 3 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[2], 0)
        self.assertAlmostEqual(autocorr[3], -3 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[4], -2 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[5], -1 / ((n_samples - 1) * var))

        # Test case III: non-trivial autocorrelation
        chains = np.array([-2, 1, 0, -1, 2])[np.newaxis, :, np.newaxis]
        autocorr = pints.diagnostics.autocorrelation(chains)

        n_samples = 6
        var = (4 + 1 + 1 + 4) / (n_samples - 1)
        self.assertEqual(autocorr.shape, (1, 5, 1))
        autocorr = autocorr[0, :, 0]
        self.assertAlmostEqual(autocorr[0], 10 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[1], -4 / ((n_samples - 1) * var))
        self.assertAlmostEqual(autocorr[2], -1 / ((n_samples - 1) * var))
...
class TestDeprecatedDiagnostics(unittest.TestCase):
    """
    Tests various diagnostic measures available in Pints
    """
    def test_autocorrelation(self):
