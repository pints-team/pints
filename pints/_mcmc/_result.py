...
class McmcResultObject(object):
    ...
    def __init__(self, chains):
        ...
        self._quantiles = np.percentile(self._stacked,[2.5,25,50,75,97.5],axis=0)
    ...
    def summary(self):
      """
      Return a list of the parameter name, posterior mean, posterior std deviation, the 2.5%, 
      25%, 50%, 75% and 97.5% posterior quantiles, rhat and effective sample size
      """
      return self._summary_list
        
    def print_summary(self):
      """
      Prints posterior summaries for all parameters to the console, including the 
      parameter name, posterior mean, posterior std deviation, the 2.5%, 
      25%, 50%, 75% and 97.5% posterior quantiles, rhat and effective sample size
      """
      print(tabulate(self._summary_list, headers = ["param", "mean", "std.", "2.5%", 
                                                     "25%", "50%", "75%", "97.5%", "rhat", "ess"], 
