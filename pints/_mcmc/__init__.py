    ...
    def run(self):
        ...
        if self._evaluations_in_memory:
            if prior:
                ...
                evals = np.zeros((self._n_chains, self._max_iterations, 3))
            else:
                # Store pdf
                evals = np.zeros(self._n_chains, self._max_iterations)
                ...
                for i in list(active):  # new list: active may be modified
                    ...
                    if y is not None:
                        ...
                        if store_evaluations:
                            ...
                            e = [current_logpdf[i]]
                            if prior is not None:
                                e.append(current_logpdf[i] - current_prior[i])
                                e.append(current_prior[i])

                        # Store evaluations in memory
                        if self._evaluations_in_memory:
                            if prior is None:
                                evaluations[i][n_samples[i]] = e[0]
                            else:
                        ...
                        if self._evaluation_files:
                            ...
                            eval_loggers[i].log(e[0])
                            if prior is not None:
                                eval_loggers[i].log(e[1])
                                eval_loggers[i].log(e[2])
