...
try:
    ...
    ErrorMsg = 'This module requires theano and pymc3 installed\n'
    ErrorMsg += 'Try pip install pymc3 and/or pip install Theano\n'
...
class SMC(pints.MCMC):
    ...
    def __init__(self, likelihood, x0, sigma0=None):
        ...
        # pass this to the _tt_evaluate functions
        ...
        # Check problem assumption: Guassian log_likelihood and should have prior
        if not isinstance(self._log_likelihood, pints.BayesianLogLikelihood):
            raise Exception('Currently only implemented for pints.BayesianLogLikelihood()')
        # Marginal Likelihood
    ...
    def _get_prior_as_pm(self, prior):
        ...
        if isinstance(prior, pints.UniformPrior):
            ...
            prior_pm = pm.Uniform('prior', 
                    lower=boundaries.lower()[:-1], 
                    upper=boundaries.upper()[:-1], 
                    transform=None, shape=self._dimension-1)
            return prior_pm
        elif isinstance(prior, pints.Prior):
            raise Exception('Provided Prior not yet implemented.')
        else:
            raise Exception('pints.Prior object is expected.')
    ...
    def run(self):
        ...
        with pm_model:
            ...
            # Guess x0
            start = [{'prior':np.random.normal(x0[:-1], x0[:-1]*sigma_x0),
        ...
        shape = pm_chain.get_values(varname[0],combine=True, squeeze=True).shape
        chain = np.zeros((shape[0],shape[1]+1))
        chain[:,:-1] = pm_chain.get_values(varname[0],combine=True, squeeze=True)
        chain[:,-1] = pm_chain.get_values(varname[1],combine=True, squeeze=True)
