...
from pints._nested._nestedRejection import reject_sample_prior
...
class NestedEllipsoidSampler(pints.NestedSampler):
    ...
    *Extends:* :class:`nestedSampler`
    ...
    def run(self):
        ...
        for i in range(0,self._iterations):
            ...
            if i > self._rejection_samples:
                if (i + 1 -self._rejection_samples) % self._ellipsoid_update_gap == 0:
                    print(str(i+1 - self._rejection_samples) + ' ellipsoidal samples completed (updating ellipsoid)...')
                    A, centroid = minimum_volume_ellipsoid(m_active[:,:d])
            ...
            if i < self._rejection_samples:
                ...
                m_active[a_min_index,:] = reject_ellipsoid_sample_faster(a_running_log_likelihood,self._log_likelihood,m_active[:,:d],self._enlargement_factor,A,centroid)
    ...
    def set_posterior_samples(self,posterior_samples):
        """
        Sets the number of posterior samples to generate from points proposed from
    ...
    def set_enlargement_factor(self,enlargement_factor):
        ...
        A higher value means it is less likely that areas of high probability mass will
        be missed. A low value means that rejection sampling is more efficient.
        """
    ...
    def set_rejection_samples(self,rejection_samples):
        """
        Sets the number of rejection samples to take, which will be assigned weights
        and ultimately produce a set of posterior samples.
        """
    ...
    def set_ellipsoid_update_gap(self,ellipsoid_update_gap):
        """
        Sets the frequency with which the minimum volume ellipsoid is re-estimated as part of the
        nested rejection sampling algorithm. A higher rate of this parameter means each
        sample will be more efficiently produced, yet the cost of re-computing the
        ellipsoid means it is often desirable to compute this every n iterates.
        """
...
def reject_uniform_draw(a_min,a_max,aLogLikelihood,threshold):
    ...
    while aLogLikelihood(a_proposed) < threshold:
        a_proposed = np.random.uniform(a_min,aMax,1)
    return np.concatenate((a_proposed,np.array([aLogLikelihood(a_proposed)])))

def reject_ellipsoid_sample(threshold,aLogLikelihood,m_samples_previous,enlargement_factor):
    ...
    if aDim > 1:
        ...
        v_sample = reject_draw_from_ellipsoid(la.inv(A),centroid,aLogLikelihood,threshold)
    else:
        a_min = np.min(m_samples_previous)
        a_max = np.max(m_samples_previous)
        a_middle = (a_min + a_max) / 2.0
        a_diff = a_max - a_min
        a_diff = a_diff * enlargement_factor
        a_min = a_middle - (a_diff / 2.0)
        a_max = a_middle + (a_diff / 2.0)
        v_sample = reject_uniform_draw(a_min,a_max,a_likelihood,aX,aN)
    return v_sample

def reject_ellipsoid_sample_faster(threshold,aLogLikelihood,m_samples_previous,enlargement_factor,A,centroid):
    """independently samples params from the prior until
        logLikelihood(params) > threshold. Accepts A as input (which is only updated every N steps)."""
    aDim = len(m_samples_previous.shape)
    if aDim > 1:
        A = (1.0 / enlargement_factor) * A
        v_sample = reject_draw_from_ellipsoid(la.inv(A),centroid,aLogLikelihood,threshold)
...
def draw_from_ellipsoid( covmat, cent, npts):
    """ draw npts random uniform points from within an ellipsoid
        with a covariance matrix covmat and a centroid cent,
        as per: http://www.astro.gla.ac.uk/~matthew/blog/?p=368"""
    try:
