...
class LogisticModelWithSensitivities(pints.ForwardModelS1):
    """
    Logistic model of population growth [1], that also provides first-order
    ...
    def __init__(self, initial_population_size=2):
        super(LogisticModelWithSensitivities, self).__init__()
        self._p0 = float(initial_population_size)
        if self._p0 < 0:
            raise ValueError('Population size cannot be negative.')
    ...
    def simulate(self, parameters, times):
        ...
        dvalues_dp[:, 1] = \
            -k * exp / (self._p0 * (c * exp + 1)**2) + 1 / (c * exp + 1)
