r0_only := proc(NextGen,gparam,gtotal,x0,p0,imap)
...
// R0_only: compute the numerical F and V
// Force infection classes to be 0 for the user
xeq:=x0:
...
fprint(Unquoted, fd2, "Numerical matrices:\nF = ",F);
fprint(Unquoted, fd2, "V = ",V,"\n");
...
// Include sensitivity equations for iteration in MATLAB -- still need this for indexing, but they will all be 0
for i from 1 to xdim do:
   dR0dx[i] := diff(R0,x[i]):
end_for:
...
fprint(Unquoted, fd, "\nR0warnings.w0 = 0;\t% since R0_only = 1");
