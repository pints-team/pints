...
class Simulation(myokit.CModule):
    ...
    def run(self, duration, log=None, log_interval=None, log_times=None,
        ...
        """
        duration = float(duration)
        output = self._run(duration, log, log_interval, log_times,
            apd_threshold, progress, msg)
        self._time += duration
        return output
    def _run(self, duration, log, log_interval, log_times, apd_threshold,
        ...
        if log_times is not None:
            ...
            if len(log_times) == 0:
                ...
                if np.any(x[1:] < x[:-1]):
                    raise ValueError('Values in log_times must be'
                        ' non-decreasing.')
                del(x, np)
        if log_times is not None and log_interval > 0:
            raise ValueError('The arguments log_times and log_interval cannot'
