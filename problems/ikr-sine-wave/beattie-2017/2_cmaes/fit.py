...
#
#TODO: Use log-likelihood instead?
...
if show:
    ...
    with np.errstate(all='ignore'): # Tell numpy not to issue warnings
        # Choose and configure optimiser
        opt = pints.CMAES(score, boundaries)

        # Run repeated optimisations
        params, scores = []
        for i in xrange(25):
            # Choose random starting point
            x0 = np.random(boundaries.lower(), boundaries.upper())
            opt.set_initial_position(x0)
